...
ggplot(long_data_1, aes(x = group, y = Expression, color = group, fill = group, shape = group)) +
  ...
  theme_minimal() +
  labs(title = "GNG4 Expressions in AD vs AsymAD (ROSMAP)",
...
ggsave(filename = file.path(path, "GNG4_Expressions_AD_vs_AsymAD_ROSMAP.tiff"),
       plot = last_plot(),
...
ggplot(long_data_2, aes(x = group, y = Expression, color = group, fill = group, shape = group)) +
  ...
  geom_jitter(size = 2, alpha = 0.7, position = position_jitter(width = 0.2, height = 0)) +
  theme_minimal() +
  labs(title = "ADAMTS2 Expressions in AD vs AsymAD (MSBB)",
       x = "Condition", y = "Expression") +
...
ggsave(filename = file.path(path, "ADAMTS2_Expressions_AD_vs_AsymAD_MSBB.tiff"),
       plot = last_plot(),
       width = 9, height = 7, device = "tiff", dpi = 600)
...
# Up-regulated genes
upregulated_genes <- subset(results_table_AD_vs_AsymAD, logFC > 0 & P.Value < 0.01)
upregulated_gene_names <- rownames(upregulated_genes)

# Down-regulated genes
downregulated_genes <- subset(results_table_AD_vs_AsymAD, logFC < 0 & P.Value < 0.01)
...
ggplot(long_data_1, aes(x = group, y = Expression, color = group, fill = group, shape = group)) +
  ...
  geom_jitter(size = 2, alpha = 0.7, position = position_jitter(width = 0.2, height = 0)) +
  theme_minimal() +
  labs(title = "MRPL1 Expressions in AD vs AsymAD (ROSMAP)",
       x = "Condition", y = "Expression") +
  scale_color_manual(values = c("AD" = "blue", "AsymAD" = "purple")) +
  scale_fill_manual(values = c("AD" = "blue", "AsymAD" = "purple")) +
  scale_shape_manual(values = c("AD" = 16, "AsymAD" = 17)) + # Circles for AD, triangles for AsymAD
...
ggsave(filename = file.path(path, "MRPL1_Expressions_AD_vs_AsymAD_ROSMAP.tiff"),
       plot = last_plot(),
       width = 9, height = 7, device = "tiff", dpi = 600)
